...
### ⚠️ 伤停权重升级（2026-09-02 复盘教训）

**案例**：米尔沃尔7人伤停，旧模型仅扣-14 Elo，实际0-3惨败。

**新规则**（已写入 `model_overrides.json` → `INJURY_ELO_SCALING`）：

| 伤停人数 | 旧扣分 | 新扣分 | 说明 |
|---------|--------|--------|------|
| ≥7人 | -14~-20 | **-40** | 防线崩盘+替补真空 |
| 5-6人 | -10~-15 | **-30** | 严重影响轮换 |
| 3-4人 | -5~-10 | **-20** | 中等影响 |
| 核心伤缺 | -25 | **-40**（叠加） | 核心+角色双重扣分 |

`scale_multiplier=2.0`，`cap=-60`。7人伤停应传 `compute_elo_delta([{"role":"star","count":1},{"role":"starter","count":2},{"role":"rotation","count":2}])`，实际返回约-38~-50。
...
## 13. 全局特殊因子（2026-09-01 新增）

基于英冠复盘数据（近均衡场次方向命中率仅41.4%、控球陷阱频发、降班马对决进球低估），以下规则**适用于所有联赛**（英超/西甲/意甲/德甲/法甲/英冠等）：

### 近均衡场次平局权重上调

**触发条件：** `elo_diff < 50`（`close_match_config.elo_diff_threshold`）

**原理：** Elo差距小的场次随机性高，模型"押主队"方向命中率在英冠仅47.8%（近均衡×押主队）。提高平局基线可更合理地反映这种不确定性。

**各联赛配置：**

| 联赛 | draw_base | draw_boost | 说明 |
|------|-----------|------------|------|
| 英冠 | 0.30 | +0.03 | 24队混战，随机性最高 |
| 英超 | 0.26 | +0.03 | 保持原draw_base，叠加close_match |
| 西甲 | 0.26 | +0.03 | 保持原draw_base，叠加close_match |
| 意甲 | 0.27 | +0.03 | 保持原draw_base，叠加close_match |
| 德甲 | 0.24 | +0.03 | 保持原draw_base，叠加close_match |
| 法甲 | 0.27 | +0.03 | 保持原draw_base，叠加close_match |

### 降班马对决进球加成

**适用范围：** 所有有降级制度的联赛（英超/西甲/意甲/德甲/法甲/英冠/德乙/西乙等）

**触发条件：** 两队均为本赛季从高级别联赛降级（`relegated_derby_config.trigger`）

| 参数 | 值 | 说明 |
|------|-----|------|
| `goal_boost` | +0.12 | 双方进球期望各+0.12 |
| `openness` | +0.08 | 比赛更开放，减少保守倾向 |

**原理：** 降班马对决有三个特征：
1. 双方防线因主力流失而不稳（如西汉姆3轮失5球、狼队3轮失4球）
2. 双方都有"反弹证明自己"的战意
3. 历史交手多次互相熟悉，不会保守

**各联赛配置：**

| 联赛 | 触发条件 | goal_boost | openness |
|------|---------|------------|----------|
| 英超 | 两队均从英超降级 | +0.12 | +0.08 |
| 西甲 | 两队均从西甲降级 | +0.12 | +0.08 |
| 意甲 | 两队均从意甲降级 | +0.12 | +0.08 |
| 德甲 | 两队均从德甲降级 | +0.12 | +0.08 |
| 法甲 | 两队均从法甲降级 | +0.12 | +0.08 |
| 英冠 | 两队均从英超降级 | +0.12 | +0.08 |
...
**触发条件（任一即触发）：**
- 控球率 > 55% **且** 场均射正 < 4次 → xG 扣减 -0.15~-0.25
- 控球率 > 60% **且** 场均射正 < 3次 → xG 扣减 -0.25~-0.35
- 控球率 > 50% **且** 射正率 < 30% → xG 扣减 -0.15~-0.25
...
{
  "tactical": {
    "ineffective_possession": true,
    "possession_efficiency": {
      "a": {"possession": 65, "shots_on_target": 3}
    }
  }
}
```

**模型自动惩罚：** 该队 xG 扣减 -0.15~-0.35（由 `compute_tactical_delta()` 计算）

**典型场景：**
- 英冠：林肯城(18射门1射正)、朴茨茅斯(21射门3射正)
- 英超：控球型球队面对大巴时的"无效传控"
- 西甲：传控球队被反击型球队克制时的低效进攻
...
## 13. 全局特殊因子（2026-09-02 新增）

除联赛特定因子外，以下全局规则适用于所有联赛，已写入 `model_overrides.json`。

### 13.1 升班马客场韧性加成（PROMOTED_AWAY_RESILIENCE）
...
**驱动模式操作**：
- 升班马客场时，`style_b` 设为 `defensive_deep`
- `expected_pattern` 设为 `cautious`
- 在 `factors[]` 中标注阵型和韧性

### 13.2 盘口降盘信号（MARKET_LINE_SIGNAL）

**案例**：米尔沃尔盘口从-0.5降至-0.25，市场早就看到问题（7人伤停+上轮1-5惨败），模型未充分采信。

**规则**：
- 降≥0.25球：平局+3%
- 降≥0.5球：平局+5%，热门-5%
- 降≥0.75球：平局+8%，热门-8%，考虑反向

**驱动模式操作**：
- 发现降盘时，在 `factors[]` 中标注 "⚠️ 盘口降盘预警"
- 手动调整 `context_openness` 增加不确定性

### 13.3 伤停权重升级（INJURY_ELO_SCALING）

详见第2节"球员可用性辅助函数"中的升级表格。

| 联赛参数表（每联赛独立） | 说明 |
|------------------------|------|
| `avg_goals` | 联赛场均进球 |
| `home_adv` | 主场加成（xG） |
| `elo_scale` | Elo→净胜球换算系数 |
| `draw_base` | 基础平局率 |
| `shock_sd` | 状态冲击标准差 |
| `probability_shrink` | 概率收缩系数 |
